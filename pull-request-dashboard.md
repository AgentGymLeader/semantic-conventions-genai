...
| [Clarify that billed token counts should be reported for Cohere usage (#211)](https://github.com/open-telemetry/semantic-conventions-genai/pull/211) ✅✅ | trask |  | ✅ | ✅ | 14h |
| [Normalize requirement-level condition notes to capitalized sentences (#245)](https://github.com/open-telemetry/semantic-conventions-genai/pull/245) ✅ | trask |  | ✅ | ✅ | 13h |
...
| [Add gen_ai.agent.finish_reason attribute for agent loop termination (#238)](https://github.com/open-telemetry/semantic-conventions-genai/pull/238) | Nik-Reddy |  | ✅ | ❌ | 16h |
| [Add gen_ai.agent.request.size and gen_ai.agent.response.size metrics (#202)](https://github.com/open-telemetry/semantic-conventions-genai/pull/202) | pvlsirotkin |  | ✅ | ✅ | 16h |
| [Add gen_ai.workflow.steps metric (#203)](https://github.com/open-telemetry/semantic-conventions-genai/pull/203) | pvlsirotkin |  | ✅ | ✅ | 15h |
| [Clarify scope of `gen_ai.client.operation.duration` metric (#215)](https://github.com/open-telemetry/semantic-conventions-genai/pull/215) | trask |  | ✅ | ❌ | 7h |
| [Add gen_ai.invoke_agent.server span (SERVER kind) (#252)](https://github.com/open-telemetry/semantic-conventions-genai/pull/252) | singankit |  | ✅ | ✅ | 4h |
| [Add Reviewers column to PR dashboard (#251)](https://github.com/open-telemetry/semantic-conventions-genai/pull/251) ✅ | trask |  | ✅ | ✅ | 2h |
| [Add prompt versioning and variable support to GenAI attributes (#179)](https://github.com/open-telemetry/semantic-conventions-genai/pull/179) | steverao | lmolkova | ✅ | ❌ | 1h |
...
PR #251
llm: PRRT_kwDOSUeMrM6HP56I -> author (The reviewer flagged a concrete workflow bug and asked for a fix; the author needs to update the PR or जवाब back.)
llm: PRRT_kwDOSUeMrM6HP56O -> author (Reviewer flagged a concrete behavior change and asked for a different notification source, so the PR author needs to update the code or respond.)

PR #250
llm: PRRT_kwDOSUeMrM6HQdgb -> author (Reviewer noted the field is not generally applicable to local agents, so the author needs to revise or respond.)
llm: PRRT_kwDOSUeMrM6HQfhz -> author (A reviewer left a nit with a suggested wording change, so the PR author needs to apply or respond to it.)
llm: PRRT_kwDOSUeMrM6HQfti -> author (A reviewer asked for links to reference docs and there is no follow-up yet, so the PR author needs to respond or make the change.)
llm: PRRT_kwDOSUeMrM6HQgJD -> author (A reviewer asked whether the shared group should appear on the internal span, and the author has not replied yet.)
llm: PRRT_kwDOSUeMrM6HQh2E -> author (Reviewer noted the item appears to be different from the documented agent invocation ID and said it isn’t documented here, so the author needs to clarify or update the PR.)
llm: PRRT_kwDOSUeMrM6HQmTV -> author (A reviewer left a substantive clarification/correction with no closing reply from the author, so the author still needs to respond or adjust the code.)
llm: PRRT_kwDOSUeMrM6HQfFg -> reviewer (The author replied with a counterpoint, so the ball is back with the reviewer to confirm or respond.)
llm: PRRT_kwDOSUeMrM6HQjux -> reviewer (The author asked for confirmation on whether reusing `gen_ai.response.id` is acceptable and said they are open to changing it, so the reviewer needs to जवाब/approve the approach.)
...
llm: PRRT_kwDOSUeMrM6HEq4S -> author (A reviewer asked for an additional scenario, so the author needs to update the PR or respond.)
...
PR #98
llm: PRRT_kwDOSUeMrM6E9NFw -> reviewer (The author answered the reviewer’s question and explained why the nesting approach doesn’t fit the SDK flow, so the ball is back with the reviewer to react or close the thread.)
llm: pr-conversation -> reviewer (The author asked the reviewer to take another pass, so the reviewer has the next action.)
