...
| [gen-ai: model agent-to-agent handoff as execute_tool span (#98)](https://github.com/open-telemetry/semantic-conventions-genai/pull/98) | Krishnachaitanyakc | lmolkova<br>MikeGoldsmith&nbsp;✅<br>trask | ✅ | ❌ | 18h |
| [Add `gen_ai.agent.finish_reason` attribute for agent loop termination (#238)](https://github.com/open-telemetry/semantic-conventions-genai/pull/238) | Nik-Reddy | aabmass&nbsp;✅<br>MikeGoldsmith&nbsp;✅ | ✅ | ✅ | 16h |
| [Add gen_ai.request.reasoning.level attribute (#258)](https://github.com/open-telemetry/semantic-conventions-genai/pull/258) | katsuhisa91 | JWinermaSplunk<br>lmolkova | ✅ | ✅ | 10h |
...
| [Add gen_ai.server.inter_token_latency metric (#164)](https://github.com/open-telemetry/semantic-conventions-genai/pull/164) | Jwrede | lmolkova&nbsp;💬 | ✅ | ❌ | 21d |
...
| [Add gen_ai.agent.invocation.duration and gen_ai.tool.execution.duration metrics (#201)](https://github.com/open-telemetry/semantic-conventions-genai/pull/201) | pvlsirotkin | lmolkova&nbsp;💬<br>MikeGoldsmith&nbsp;🔴 | ✅ | ❌ | 9d |
...
| [Add gen_ai.invoke_agent.server span (SERVER kind) (#252)](https://github.com/open-telemetry/semantic-conventions-genai/pull/252) | singankit | Copilot&nbsp;💬 | ✅ | ✅ | 4d |
...
| [Limit supported  part types  on `gen_ai.system_instructions` to text only (#257)](https://github.com/open-telemetry/semantic-conventions-genai/pull/257) | lmolkova | MikeGoldsmith&nbsp;💬⁠✅ | ✅ | ✅ | 22h |
| [gen-ai: add run guardrail span and security finding (#262)](https://github.com/open-telemetry/semantic-conventions-genai/pull/262) | nagkumar91 | aabmass<br>Copilot&nbsp;💬 | ✅ | ✅ | 17h |
| [Add workflow node convention (#188)](https://github.com/open-telemetry/semantic-conventions-genai/pull/188) | RKest | aabmass&nbsp;💬<br>lmolkova&nbsp;🔴<br>trask&nbsp;💬 | ✅ | ❌ | 16h |
| [semconv for a2a protocol (#195)](https://github.com/open-telemetry/semantic-conventions-genai/pull/195) | eternalcuriouslearner | aabmass&nbsp;💬<br>JWinermaSplunk&nbsp;💬<br>pwkowalski&nbsp;💬<br>trask&nbsp;💬 | ✅ | ✅ | 7h |

## Waiting on external

| PR | Author | Reviewers | CI | Conflicts | Age |
|---|---|---|:---:|:---:|:---:|
| [Update dependency google-genai to v2 (#112)](https://github.com/open-telemetry/semantic-conventions-genai/pull/112) | app/renovate | DylanRussell&nbsp;💬<br>lmolkova&nbsp;✅ | ❌ | ✅ | 20d |
...
| [Add time_budget value for gen_ai.agent.finish_reason (#267)](https://github.com/open-telemetry/semantic-conventions-genai/pull/267) | Nik-Reddy | 6h |
...
```text
PR #262
llm: PRRT_kwDOSUeMrM6H7_Uk -> author (Reviewer flagged a misleading hard-coded hash and requested it be computed from the chosen input or the constant be updated, so the PR author needs to act.)
llm: PRRT_kwDOSUeMrM6H7_VR -> author (A reviewer pointed out that the hard-coded hash value is misleading and asked for it to be computed from the intended input or updated to match the scenario, so the PR author needs to act.)
llm: PRRT_kwDOSUeMrM6H7_Vr -> author (A reviewer raised a concrete change request and no author reply has been posted, so the PR author needs to update the span definitions or respond.)
llm: PRRT_kwDOSUeMrM6H7_Vz -> author (A reviewer raised a concrete follow-up suggestion and the thread is still unresolved, so the PR author needs to respond or make the change.)
...
PR #238
llm: pr-conversation -> reviewer (The author has already replied, addressed the suggestion, and updated the snapshot, so the ball is back with the reviewer/maintainer to re-review or merge.)
...
PR #201
llm: PRRT_kwDOSUeMrM6FYxtF -> author (The reviewer raised a documentation gap, and the author replied that they will add the explanatory sentence, so the next action is on the author.)
...
llm: PRRT_kwDOSUeMrM6FY5Vy -> none (The author addressed the reviewer’s suggestion by adding `gen_ai.tool.type` at `recommended`, so no further follow-up is needed in this thread.)
...
llm: PRRT_kwDOSUeMrM6H93N8 -> author (A reviewer asked whether GenAI frameworks/libs have a notion of tool version and pointed to existing version reporting, so the author needs to जवाब/respond or adjust the PR.)
...
llm: PRRT_kwDOSUeMrM6H9_Oc -> author (The latest comment is from a reviewer proposing a metric name change and saying the PR can start with `gen_ai.invoke_agent.duration` only, so the author needs to respond or implement that feedback.)
...
PR #195
llm: pr-conversation -> author (The last substantive comment is from a reviewer offering to help and asking the author to let them know, so the ball is with the author to respond or proceed.)
...
PR #164
llm: PRRT_kwDOSUeMrM6C-3Kb -> author (A reviewer asked a substantive question and suggested changing the metric approach; the author needs to जवाब/respond and either justify the new metric or adjust the definition.)
llm: pr-conversation -> author (The latest comment is from the reviewer and asks the author to clarify the metric description in OTel terms, so the author needs to respond or update the PR.)
