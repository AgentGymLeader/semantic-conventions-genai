...
| PR | Author | Reviewers | CI | Conflicts | Age |
|---|---|---|:---:|:---:|:---:|
| [Update dependency google-adk to v2 (#173)](https://github.com/open-telemetry/semantic-conventions-genai/pull/173) | app/renovate | MikeGoldsmith&nbsp;💬 | ❌ | ✅ | 19d |
| [semconv for a2a protocol (#195)](https://github.com/open-telemetry/semantic-conventions-genai/pull/195) | eternalcuriouslearner | aabmass | ✅ | ✅ | 9d |
| [Clarify that billed token counts should be reported for Cohere usage (#211)](https://github.com/open-telemetry/semantic-conventions-genai/pull/211) | trask | lmolkova&nbsp;✅<br>MikeGoldsmith&nbsp;💬⁠✅ | ✅ | ✅ | 2d |
...
| [Update flint to v0.22.5 (#259)](https://github.com/open-telemetry/semantic-conventions-genai/pull/259) | app/renovate |  | ✅ | ✅ | 14h |
| [Avoid repeated initial PR dashboard Slack notifications (#261)](https://github.com/open-telemetry/semantic-conventions-genai/pull/261) | trask |  | ✅ | ✅ | 11h |
| [Limit PR dashboard Slack follow-ups to weekdays (#260)](https://github.com/open-telemetry/semantic-conventions-genai/pull/260) | trask |  | ✅ | ✅ | 11h |
...
| [Add workflow node convention (#188)](https://github.com/open-telemetry/semantic-conventions-genai/pull/188) | RKest | lmolkova&nbsp;🔴<br>trask&nbsp;💬 | ✅ | ❌ | 8d |
...
| [semconv for compaction (#162)](https://github.com/open-telemetry/semantic-conventions-genai/pull/162) | eternalcuriouslearner | JWinermaSplunk<br>lmolkova&nbsp;💬⁠✅<br>trask | ✅ | ✅ | 6d |
...
| [Limit gen_ai.agent.id to stable / static identifiers (#242)](https://github.com/open-telemetry/semantic-conventions-genai/pull/242) | lmolkova | aabmass&nbsp;💬⁠✅<br>MikeGoldsmith&nbsp;💬⁠✅ | ✅ | ✅ | 3h |
...
| [Add gen_ai.agent.invocation.id attribute for invoke_agent spans (#250)](https://github.com/open-telemetry/semantic-conventions-genai/pull/250) | singankit | lmolkova&nbsp;💬<br>MikeGoldsmith&nbsp;🔴 | ✅ | ❌ | 1h |
| [Limit supported  part types  on `gen_ai.system_instructions` to text only (#257)](https://github.com/open-telemetry/semantic-conventions-genai/pull/257) | lmolkova | MikeGoldsmith&nbsp;💬⁠✅ | ✅ | ✅ | 42m |
| [Add gen_ai.agent.finish_reason attribute for agent loop termination (#238)](https://github.com/open-telemetry/semantic-conventions-genai/pull/238) | Nik-Reddy | MikeGoldsmith&nbsp;💬⁠✅ | ✅ | ✅ | 20m |
...
```text
PR #258
llm: pr-conversation -> none (The author says all feedback was addressed and thanks the reviewer, so the thread appears closed with no further action needed.)
...
llm: PRRT_kwDOSUeMrM6HzsI1 -> author (Reviewer raised a concrete nit about the GCP identifier, so the author needs to adjust or जवाब/reply; no follow-up from the author yet.)

PR #238
llm: PRRT_kwDOSUeMrM6H3o_A -> author (Reviewer requested a wording change in `model/gen-ai/spans.yaml`; the author needs to apply or respond to the suggestion.)
llm: pr-conversation -> none (The reviewer’s last comment is an approval and simple thanks, which closes the thread with no follow-up needed.)
...
llm: pr-conversation -> none (Latest reviewer comment says it is ready for merge and does not request any follow-up.)
...
PR #162
llm: PRRT_kwDOSUeMrM6GRukM -> author (A reviewer asked a follow-up question about whether to reference it on the operation duration metric, so the author needs to जवाब/act.)
...
PR #98
llm: PRRT_kwDOSUeMrM6E9NFw -> author (The latest reviewer/approver comment asks to update the registry entry to reflect the chosen hierarchy, so the PR author needs to make that change or respond with the decision.)
llm: PRRT_kwDOSUeMrM6H1QMV -> author (A reviewer said the extra paragraph likely isn’t needed, so the author needs to respond or adjust the file.)
llm: PRRT_kwDOSUeMrM6H1hSs -> author (A reviewer raised a documentation concern and asked a question; the author needs to respond or update the PR.)
