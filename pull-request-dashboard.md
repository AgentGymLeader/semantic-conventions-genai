...
| [Add gen_ai.agent.request.size and gen_ai.agent.response.size metrics (#202)](https://github.com/open-telemetry/semantic-conventions-genai/pull/202) | pvlsirotkin | lmolkova<br>MikeGoldsmith&nbsp;🔴<br>trask | ✅ | ✅ | 30m |
...
| [Add gen_ai.workflow.steps metric (#203)](https://github.com/open-telemetry/semantic-conventions-genai/pull/203) | pvlsirotkin | aabmass&nbsp;💬⁠✅<br>lmolkova&nbsp;💬<br>MikeGoldsmith&nbsp;🔴<br>trask | ✅ | ✅ | 6d |
...
| [semconv for a2a protocol (#195)](https://github.com/open-telemetry/semantic-conventions-genai/pull/195) | eternalcuriouslearner | aabmass<br>JWinermaSplunk<br>trask | ✅ | ❌ | 23h |
...
| [Add Towncrier changelog management (#275)](https://github.com/open-telemetry/semantic-conventions-genai/pull/275) | trask | Copilot&nbsp;💬<br>lmolkova&nbsp;✅ | ✅ | ✅ | 10m |

## Waiting on external

| PR | Author | Reviewers | CI | Conflicts | Age |
|---|---|---|:---:|:---:|:---:|
| [Update dependency google-genai to v2 (#112)](https://github.com/open-telemetry/semantic-conventions-genai/pull/112) | app/renovate | lmolkova&nbsp;✅ | ❌ | ✅ | 21d |
...
```text
PR #275
llm: PRRT_kwDOSUeMrM6Ihs04 -> author (A reviewer flagged that the glob is too broad and suggested narrowing it, so the PR author needs to update the instruction file or respond.)

PR #270
llm: PRRT_kwDOSUeMrM6IUWvu -> none (The author acknowledged the suggestion, asked one clarifying question, then reported it was added in commit 0bc77a0, so the thread appears resolved.)
llm: PRRT_kwDOSUeMrM6IW2DR -> author (The reviewer flagged a concrete bug in `snippet.md.j2` and hasn’t received a response yet, so the author needs to fix or reply.)
llm: PRRT_kwDOSUeMrM6IW2De -> author (A reviewer pointed out a likely config bug (`template:` vs `pattern:`) and no one has replied yet, so the author needs to respond or fix it.)
llm: PRRT_kwDOSUeMrM6IW2Dq -> author (A reviewer asked for a change to `templates/registry/markdown/weaver.yaml` and no author response is present, so the PR author needs to act.)
llm: PRRT_kwDOSUeMrM6IW2Dx -> author (The reviewer asked for a change or PR description update, so the author needs to respond and act on it.)
llm: pr-conversation -> author (A reviewer asked for the two items to be bundled together for easier review, so the author needs to respond or update the PR.)
...
PR #238
llm: pr-conversation -> author (The last comment is a reviewer giving a substantive answer and +1 on the proposed scope, so the ball is back with the author to acknowledge or act on it.)
...
PR #202
llm: PRRT_kwDOSUeMrM6HFBTV -> reviewer (The author replied to the review question, explained why `attributes.gen_ai.error` was added and why `metric_attributes.gen_ai` was deferred, so the reviewer now needs to accept or push back if they disagree.)

PR #201
llm: PRRT_kwDOSUeMrM6FY1gg -> reviewer (The author asked whether `recommended` or `required` would be better and said they’re happy to change it, so the ball is back with the reviewer to answer/decide.)
llm: PRRT_kwDOSUeMrM6FY3VY -> reviewer (The author asked for confirmation on the approach and whether metric validation can be wired up, so the reviewer/maintainer needs to answer or clarify next.)
llm: PRRT_kwDOSUeMrM6H95-W -> external (The reviewer is waiting on a GenAI call/issue discussion to confirm the naming pattern before updating the thread.)
llm: PRRT_kwDOSUeMrM6H9_Oc -> author (The reviewer suggested aligning the metric name and scoped the PR to `gen_ai.invoke_agent.duration`, which leaves the author to acknowledge or apply that guidance.)
