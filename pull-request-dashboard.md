...
| [Add gen_ai.request.reasoning.level attribute (#258)](https://github.com/open-telemetry/semantic-conventions-genai/pull/258) | katsuhisa91 | JWinermaSplunk<br>lmolkova | ✅ | ✅ | 23m |
...
| [Add `gen_ai.agent.finish_reason` attribute for agent loop termination (#238)](https://github.com/open-telemetry/semantic-conventions-genai/pull/238) | Nik-Reddy | aabmass&nbsp;✅<br>AgentGymLeader&nbsp;💬<br>Copilot&nbsp;💬<br>MikeGoldsmith&nbsp;✅<br>trask&nbsp;💬 | ✅ | ✅ | 5h |
...
| [Support entity rendering in Weaver v2 (#273)](https://github.com/open-telemetry/semantic-conventions-genai/pull/273) | aabmass | lmolkova&nbsp;💬 | ✅ | ✅ | 47m |
...
| [Add time_budget value for gen_ai.agent.finish_reason (#267)](https://github.com/open-telemetry/semantic-conventions-genai/pull/267) | Nik-Reddy | 16h |
| [Propose GenAI agent entity (#270)](https://github.com/open-telemetry/semantic-conventions-genai/pull/270) | aabmass | 5m |
...
```text
PR #273
llm: PRRT_kwDOSUeMrM6ISXzp -> author (The last comment is from a reviewer raising a substantive point about resource definitions, so the author needs to जवाब/respond or adjust the PR.)
llm: PRRT_kwDOSUeMrM6ITvV8 -> author (A reviewer/approver pointed out the template won’t work and identified missing context/attributes, so the PR author needs to fix or respond.)
llm: PRRT_kwDOSUeMrM6ITv0y -> author (A reviewer asked a direct question and no one has پاسخed yet, so the author needs to respond.)
llm: PRRT_kwDOSUeMrM6IThaB -> reviewer (The author replied with an acknowledgement, so per the default heuristic the ball is back with the reviewer even though the underlying schema concern wasn’t explicitly resolved.)
llm: PRRT_kwDOSUeMrM6ITtUs -> reviewer (The author asked a question about whether the entity registry is needed, so the reviewer/maintainer needs to answer or confirm the intended direction.)

PR #262
llm: PRRT_kwDOSUeMrM6ISkfg -> author (A reviewer asked to change the `gen_ai.security.finding` link target for consistency, so the PR author needs to update the reference.)
llm: PRRT_kwDOSUeMrM6ISkgL -> author (A reviewer flagged a likely KeyError from `os.environ["MOCK_LLM_URL"]` and suggested a fix; there’s no author follow-up yet, so the author needs to respond or update the scenario.)
llm: PRRT_kwDOSUeMrM6ISkge -> author (A reviewer pointed out that `server.address` may be set without `server.port`, which could violate the convention; the PR author needs to update the scenario or explain the behavior.)
llm: PRRT_kwDOSUeMrM6ISkgy -> author (The reviewer raised a concrete concern and suggested a change; the author needs to respond or update the reference implementation.)
llm: PRRT_kwDOSUeMrM6ISkhF -> author (The reviewer raised a concrete code-level suggestion about using `span_kind` as the primary signal, so the PR author needs to respond or implement a change.)
...
llm: pr-conversation -> author (The latest comment is from a reviewer giving a substantive recommendation on scope and finish_reason framing, so the ball is back with the PR author to respond or adjust the PR.)
