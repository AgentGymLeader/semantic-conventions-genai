...
| [chore: add moonshot_ai, minimax, z_ai to well-known values (#99)](https://github.com/open-telemetry/semantic-conventions-genai/pull/99) | ariesdevil |  | ✅ | ✅ | 18m |
...
| [Disable PR preview for webhook (#176)](https://github.com/open-telemetry/semantic-conventions-genai/pull/176) ✅ | trask |  | ✅ | ✅ | 16h |
| [genai: add `gen_ai.token.cache` and `gen_ai.token.reasoning` metric attributes (#96)](https://github.com/open-telemetry/semantic-conventions-genai/pull/96) | Nik-Reddy | trask | ✅ | ❌ | 1h |
| [gen-ai: add gen_ai.response.id to deepeval evaluation result event (#184)](https://github.com/open-telemetry/semantic-conventions-genai/pull/184) | hippoley |  | ❌ | ✅ | 1h |
| [gen-ai: add evaluation operation name and gen_ai.evaluate.internal span (#185)](https://github.com/open-telemetry/semantic-conventions-genai/pull/185) | hippoley |  | ❌ | ✅ | 58m |
...
```text
PR #185
llm: PRRT_kwDOSUeMrM6DuuPM -> author (A reviewer raised a terminology inconsistency and asked to align the naming, so the PR author needs to respond or make the change.)
...
llm: PRRT_kwDOSUeMrM6DuuP5 -> author (Reviewer suggested a formatting change and no author reply has addressed it yet.)
...
llm: PRRT_kwDOSUeMrM6Dun6t -> author (The reviewer requested a configurable override for the hard-coded mock response ID, so the author needs to respond by implementing or addressing that suggestion.)
...
PR #96
llm: PRRT_kwDOSUeMrM6Ck7X- -> author (The latest author reply explicitly says they are holding off on patching modality until the shape settles, so the author still owns the next action.)
